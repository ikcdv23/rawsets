...
- **Tipografía**: `font-display` (ZenDots) **solo para el wordmark del logo**, nunca en UI. Para todo el resto: `font-sans` (Inter Regular) cuerpo, `font-sans-medium`/`bold`/`black` para énfasis y títulos, `font-mono`/`mono-bold` (JetBrains Mono) para números, pesos, métricas, IDs.
