...
export class DrizzleSqliteScheduledSessionRepo implements ScheduledSessionRepo {
  ...
  async listInRange(from: Date, to: Date): Promise<ScheduledSession[]> {
  ...
  async upsertOnDate(session: ScheduledSession): Promise<void> {
  ...
  async removeByDate(date: Date): Promise<void> {
    await this.sqlite.runAsync('DELETE FROM scheduled_sessions WHERE date = ?', [
