...
export function useRoutineDetail(id: string | undefined) {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      if (!rRes.ok) console.error('[useRoutineDetail] findById error:', rRes.error);
      if (!catRes.ok) console.error('[useRoutineDetail] listExercises error:', catRes.error);
    } catch (err) {
  ...
  const handleUpdateExercise = useCallback(
    async (patch: Partial<RoutineExercise>) => {
      ...
      const res = await updateRoutineExercise(routineRepo, routine.id, editing.exerciseId, patch);
      if (res.ok) {
        await reload();
        setEditing(null);
      } else {
  ...
  const handleMoveExercise = useCallback(
    async (exerciseId: string, direction: 'up' | 'down') => {
      if (!routine) return;
      const res = await moveExerciseInRoutine(routineRepo, routine.id, exerciseId, direction);
      if (res.ok) {
        await reload();
      } else {
