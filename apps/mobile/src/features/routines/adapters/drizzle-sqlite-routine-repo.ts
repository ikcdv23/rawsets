...
export class DrizzleSqliteRoutineRepo implements RoutineRepo {
  ...
  async create(routine: Omit<Routine, 'createdAt'>): Promise<void> {
    const now = Date.now();
  ...
  async rename(id: string, name: string): Promise<void> {
    await this.sqlite.runAsync('UPDATE routines SET name = ? WHERE id = ?', [name, id]);
  }

  async delete(id: string): Promise<void> {
    await this.sqlite.runAsync('DELETE FROM routines WHERE id = ?', [id]);
  }

  async setExercises(routineId: string, exercises: RoutineExercise[]): Promise<void> {
    await this.sqlite.runAsync('DELETE FROM routine_exercises WHERE routine_id = ?', [routineId]);
